...
## 8.23 Progressive wallet-list reachability

`<stellar-appkit-modal>` paints the FULL wallet list the instant `open()` runs — every row in a neutral "Checking…" state — and settles each row in place the moment its own `getReachability()` resolves (`ui-web/wallet-list.ts`, driven by the modal's `refreshWalletList()`). Before this, the modal waited on `ConnectorRegistry.listReachability()`'s `Promise.all` before rendering a single row: one slow check held the whole list hostage. Freighter's installed-check rides a 3-second timeout when the extension isn't present, so a user opening the modal shortly after page load (before the attach-time prefetch finished) stared at "Loading wallets…" for the full window — and reported it as "the modal takes 5-10 seconds to open". The modal itself was painting in <40ms the whole time.

Design points:

- **Row sorting is stable** — the initial paint sorts exactly like the registry does (WalletConnect pinned first, registration order otherwise) and rows update in place, so nothing visibly re-orders as reachability lands.
- **Clicking a 'checking' row is safe** — `selectWallet()` re-resolves reachability at click time before doing anything, so a premature tap pays the check once and routes correctly (connect, install URL, or nothing for unavailable wallets).
- **'checking' rows are never dimmed or disabled** — the 0.55 `data-unavailable` dimming and the disabled gate fire only for known-bad states (`locked`/`unavailable`).
- **Superseded batches are cancelled** — `refreshWalletList()` stores the tracker's cancel function and calls it on the next refresh and on `disconnectedCallback()`, so stale rows can never paint over a fresh list or into a torn-down shadow DOM.
- `listReachability()` itself is unchanged — headless apps still get the aggregate one-shot API; the progressive flow is the modal's own surface.
- The `checking` i18n key ships in all 25 locales (`wallet_list.status.checking`).
...
| 3 | `react-native` — `@saganta/stellar-appkit-react-native` (subpath exports `./ui`, `./albedo`, `./polyfills`, mirroring core's wrapper pattern). Headless entry re-exports all of core plus `defaultReactNativeConnectors()` (WalletConnect relay + optional Albedo WebView), `createAsyncStorage()` persistence, an MWA-style mobile-wallet deep-link registry (built-ins verified against the WalletConnect Explorer: Freighter `freighterwallet://`, LOBSTR `lobstr://`, HOT Wallet `hotwallet://`, Scopuly `scopuly://wc`, plus https universal-link fallbacks; extensible via `registerMobileWallet()`), and `isReactNativeRuntime()`. `./ui` ships the full modal in two presentations — `AppKitModal` on `@gorhom/bottom-sheet` (default) or `mode="inline"` (web `mode="inline"` parity: bordered panel, no overlay/close) — as an orchestrator over one file per screen under `src/ui/views/` (HeaderView with the web's three variants, WalletListView, WalletRowView, ConnectingView, SigningView, SiwsView, AccountView, ErrorView + NetworkMismatchView, sharing a web-parity design system in `src/ui/styles.ts`). UI is 1:1 with the web modal's specs: the web's squircle dash-arc spinner (the traveling-dash rounded square — NOT a circle — reproduced with pure Views via `squircle-track.ts` exact SVG dash math + native-driven per-segment opacity PWM, `SquircleArc.tsx`; 2s connecting / 0.8s signing, reduced-motion aware), the `.header--connecting` back-arrow error header, web error routing (failed connects stay on the connecting view's error variant, rejected signs on the signing view, NetworkMismatchError → wrong-network view), staggered entrance animations, the "Powered by Stellar AppKit" footer, and the full SIWS flow (`useSiws.ts` — port of the web `triggerSiwsFlow`: checking → nonce → signing → verifying, per-step timeouts, retry caps, disconnect-on-fail). Auto-close on success (`ui/auto-close.ts`): when the operation the app requested completes *successfully* — a connect settles (deep-link return from the wallet app included, SIWS run to its end) or the sign queue drains cleanly — the sheet closes itself after a ~0.9s confirmation flash, the deliberate RN deviation from the web modal's stay-open account view (failures/rejections NEVER auto-close — web error-routing parity; every user navigation disarms the flag so a reopened modal can never self-close; opt out with `autoCloseOnComplete={false}`). Chrome icons are pure-View ports of the web's inline SVGs (`icons.tsx`). Named mobile wallet list with per-wallet branding — the connecting/account views show the peer wallet's real name/icon via `getSessionPeer()`, never a generic “WalletConnect” label — plus a `<WalletIcon>` that renders pre-rasterized compressed PNG icons (react-native-svg fully eliminated) with name-alias registry lookup and letter-avatar fallbacks, deep-link-only WC pairing (a phone can't scan its own screen, so no QR is ever shown; `QrCodeView` remains exported for tablet use cases), a wallet list that mirrors the web modal's flat-row design (40dp squircle tiles with drop shadow, “Installed” outline badges with accent dots, accent Install pills, 0.55 dimming for unavailable wallets), v1.9.50 animation timings with AccessibilityInfo reduced-motion, all 10 themes, core i18n; `./albedo` bridges Albedo's web confirm flow through an origin-locked `react-native-webview`; `./polyfills` installs Buffer + `crypto.getRandomValues` + WC RN compat in one call. Core made Metro-safe (pre-encoded icon constants, adaptive WebCrypto SHA-256 with js-sha256 fallback). Deferred WC warm-up (ui/warm-up.ts): the modal warms the SignClient at mount (app start for the always-mounted pattern) and again on open — each schedule is gated on `InteractionManager.runAfterInteractions()` plus a settle timer: ~150ms for the open trigger and ~2s for the mount trigger. The SDK module-tree evaluation blocks the JS thread for seconds on debug builds; firing it in the sheet-open tick froze the sheet's layout/entrance animation (Connect tap dead 5-10s), and firing it in the app's first ticks froze the freshly-painted screen's every button for the same window — the settle windows and the interaction gate keep both triggers clear of the user's interactions, and the RN README documents the eager-preload pattern (`import('@walletconnect/sign-client')` at the top of the entry file, paid once behind the splash) for a zero-freeze startup. Auto-connect (`StellarAppKitConfig.autoConnect`) schedules `restore()` in the constructor; on React Native that schedule is deferred ~1.2s (`isReactNativeLikeRuntime()` in core — navigator.product / HermesInternal, the same signals `isReactNativeRuntime()` reads in the RN package; web restores immediately) because with a persisted WC session the restore chain evaluates the whole WC SDK synchronously and must never race the app's first paint. Mobile focus-return (src/focus-return.ts): `appMetadata.redirect` (core config, WC/Reown metadata standard) rides every WC session proposal so cooperating wallets bounce the user back to the dapp after approve/reject, and the modal's `useAppFocusReturn` — `attachAppFocusReturn()` for headless apps — best-effort re-opens the app's own deep link when an operation (connect or sign, success OR failure) settles while the app is backgrounded behind the wallet app; attempts are gated on AppState != 'active' (in-app outcomes never steal focus), a connecting-latch covers failed connects (setStatus('error') fires before the error event), a 1s cooldown dedupes a failed sign's error+queue double-report, and OS refusals (iOS background self-open) are swallowed. 241 package tests (incl. squircle-geometry, web-parity style guards, focus-return policy, auto-close rules, interaction-gated warm-up); total suite 688. Runnable Expo Go demo (no native build): [SagantaHQ/stellar-appkit-RN-demo](https://github.com/SagantaHQ/stellar-appkit-RN-demo). | **done** (on npm since v1.9.51) |
