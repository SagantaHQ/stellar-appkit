/**
 * shouldAutoClose — the pure half of the auto-close-on-success behavior
 * (ui/auto-close.ts; the wiring lives in AppKitModal.tsx).
 *
 * WHAT THIS PINS: when an operation the app requested through the modal
 * completes SUCCESSFULLY (a connect settles after the deep-link handoff,
 * or the sign queue drains cleanly), the sheet closes itself — but ONLY
 * then. Success is the single trigger. The regressions these tests guard:
 *
 ...
 * - errors / rejections / expired requests / in-flight views never
 *   auto-close (web parity — the user reads the outcome and acts on it)
...
describe('shouldAutoClose — the success case (sheet slides away)', () => {
  test('armed connect/sign SUCCESS on the account view closes', () => {
...
describe('shouldAutoClose — what must NEVER auto-close (success is the ONLY trigger)', () => {
