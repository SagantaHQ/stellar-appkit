...
export const StellarAppKitModal = defineComponent({
  ...
  props: {
    mode: { type: String as PropType<StellarAppKitModalProps['mode']>, default: undefined },
    theme: { type: String as PropType<StellarAppKitModalProps['theme']>, default: undefined },
    branding: { type: String as PropType<StellarAppKitModalProps['branding']>, default: undefined },
    logoSrc: { type: String as PropType<StellarAppKitModalProps['logoSrc']>, default: undefined },
    title: { type: String as PropType<StellarAppKitModalProps['title']>, default: undefined },
    autoRetryNetwork: { type: Boolean as PropType<StellarAppKitModalProps['autoRetryNetwork']>, default: undefined },
    stellarExpertAvatars: { type: Boolean as PropType<StellarAppKitModalProps['stellarExpertAvatars']>, default: undefined },
  },
  emits: {
    connect: (_session: ConnectSession) => true,
    disconnect: (_payload: { walletId: string }) => true,
  ...
  setup(props, { expose, emit, attrs }) {
    ...
    const hostRef = ref<HTMLElement & { client: typeof client | null } | null>(null);
    ...
    onMounted(() => {
      ...
      const onConnect = (e: Event) => emit('connect', (e as CustomEvent).detail);
      const onDisconnect = (e: Event) => emit('disconnect', (e as CustomEvent).detail);
      const onError = (e: Event) => emit('error', (e as CustomEvent).detail);
      ...
      cleanupHandlers.push(
        () => el.removeEventListener('sc-connect', onConnect),
        () => el.removeEventListener('sc-disconnect', onDisconnect),
        () => el.removeEventListener('sc-error', onError),
      );
    });
    ...
    expose({
      open: async () => {
        await (hostRef.value as unknown as { open?: () => Promise<void> })?.open?.();
      },
      close: () => {
        (hostRef.value as unknown as { close?: () => void })?.close?.();
      },
      get element() {
        return hostRef.value as HTMLElement & { client: typeof client | null };
      },
