...
export async function buildTransactionPreview(xdr, networkPassphrase, opts = {}) {
    ...
    switch (op.type) {
        ...
        case 'setOptions': {
            ...
            if (op.masterWeight !== undefined)
                changes.push(`set master key weight to ${op.masterWeight}`);
...
function decodeInvokeHostFunction(op, sdk, opts) {
    const flags = [];
    const details = {};
    ...
    if (op.func.switch().name === 'hostFunctionTypeInvokeContract') {
        ...
        const functionName = typeof invoke.functionName() === 'string' ? invoke.functionName() : invoke.functionName().toString();
...
function checkVerified(contractId, verified) {
    if (!verified)
        return null; // no verification source configured — skip the check rather than flag everything
    if (typeof verified === 'function')
        return verified(contractId);
